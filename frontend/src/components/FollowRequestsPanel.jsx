...
import { getFollowRequests, acceptFollowRequestFromUser, rejectFollowRequestFromUser } from '../api/users';
...
export default function FollowRequestsPanel() {
    ...
    const handleAccept = async (userId) => {
        try {
            await acceptFollowRequestFromUser(userId);
    ...
    const handleReject = async (userId) => {
        try {
            await rejectFollowRequestFromUser(userId);
            setRequests(prev => prev.filter(r => r.userId !== userId));
        } catch (err) {
            console.error(err);
    ...
    return (
        <div>
            <h3 className="text-[var(--text-primary)] font-semibold mb-4 flex items-center gap-2">
                ...
                Follow Requests
                {requests.length > 0 && (
                    <span className="px-2 py-0.5 text-xs font-bold bg-[var(--danger)] text-white rounded-full">
                        {requests.length}
                    </span>
                )}
            </h3>
                ...
                <AnimatePresence>
                    {requests.map(user => (
                        <motion.div
                            ...
                            className="flex items-center justify-between p-3 rounded-lg hover:bg-[var(--bg-elevated)] mb-1"
                        >
                            ...
                            <div className="flex gap-2">
                                <motion.button
                                    ...
                                    className="px-4 py-1.5 text-sm font-semibold text-white bg-[var(--accent)] rounded-lg hover:bg-[var(--accent-hover)] transition-colors cursor-pointer"
                                >
                                    <Check size={16} />
                                </motion.button>
                                <motion.button
